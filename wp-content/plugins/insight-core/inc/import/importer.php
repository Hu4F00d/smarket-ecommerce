	...
	class InsightCore_Importer extends WP_Importer {
		...
		function download_package( $_tmppath ) {
			...
			if ( ! is_dir( $_tmppath ) ) {
				...
				if ( $this->importFullDemo ) {
					echo '<script type="text/javascript">progress_status("dl");text_status(\'' . sprintf( wp_kses( __( 'Downloading package... You can download the package via <a href="%s" target="_blank">this direct link</a>.' ), array(
							'strong' => array(),
							'a'      => array(
								'href'   => array(),
								'target' => array(),
