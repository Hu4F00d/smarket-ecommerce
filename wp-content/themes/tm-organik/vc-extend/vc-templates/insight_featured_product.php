...
$style_img = '';
if ( ! empty( $image ) && is_string( $image ) && 'style-07' != $style ) {
	$style_img = 'background-image: url("' . $image . '");';
}
...
?>
<div id="<?php echo esc_attr( $selector ) ?>" class="insight-featured-product <?php echo esc_attr( $el_class ) ?>"
	...
	<script>
		jQuery( document ).ready( function() {
			...
			countdown.setLabels(
				...
				'<?php echo '' . $last ?>',
				'<?php echo '' . $delim ?>',
				'<?php echo esc_html( 'End', 'tm-organik' ) ?>',
				function( n ) {
					if ( n < 10 ) {
						return '0' + n.toString();
					}
